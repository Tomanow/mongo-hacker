...
DB.prototype.indexStats = function(collectionFilter, details) {
  ...
  documentIndexes = collectionNames.map(function(collectionName) {
    const count = db.getCollection(collectionName).count();
  ...
  for (i = 0; i < collectionNames.length; i++) {
    print(
        colorize(collectionNames[i].pad(maxKeyLength, true),
            mongo_hacker_config['colors']['collectionNames'])
        + ' ' + columnSeparator + ' '
        + documentIndexes[i].pad(maxValueLength)
    );

    const stats = db.getCollection(collectionNames[i]).stats();
    const totalIndexSize = (Math.round(
        (stats.totalIndexSize / 1024 / 1024) * 10) / 10) + ' MB';
    ...
    print(
        colorize('totalIndexSize'.pad(maxKeyLength, true),
            mongo_hacker_config['colors']['string'])
        + ' ' + columnSeparator + ' '
        + colorize(totalIndexSize.pad(maxValueLength),
            mongo_hacker_config['colors']['number'])
    );

    if (details) {
      for (let j = 0; j < indexSizes.length; j++) {
        print(
            colorize('' + indexNames[j].pad(maxIndexKeyLength, true),
                mongo_hacker_config['colors']['string'])
            + ' ' + columnSeparator + ' '
            + colorize(indexSizes[j].pad(maxIndexValueLength),
...
DBQuery.prototype.upsert = function(upsert) {
  ...
  const startTime = (typeof (_verboseShell) === 'undefined' || !_verboseShell)
                    ? 0
                    : new Date().getTime();
...
DBQuery.prototype.textSearch = function(search) {
  ...
  const result = this._db.runCommand(text);
