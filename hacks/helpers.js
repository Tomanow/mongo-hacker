...
function getEnv(env_var) {
  const env_regex = new RegExp(' ' + env_var + '=(.*)');
  return runMatch('env', '', env_regex)[1];
}

function getVersion() {
  const regexp = /version: (\d).(\d).(\d)/;
...
function getSlowms() {
  if (!isMongos()) {
    const current_version = parseFloat(db.serverBuildInfo().version).toFixed(2);
    if (current_version >= 4.0) {
      // The `profile` command can block with pending transactions in 4.0+ (#193)
      return 100;
    }

    const res = db._dbCommand({profile: -1});
...
function printPaddedColumns() {
  const columnWidths = Array.prototype.map.call(arguments, function(column) {
  ...
  for (i = 0; i < arguments[0].length; i++) {
    ...
    for (j = 0; j < arguments.length; j++) {
      let separator = '';
      let val = arguments[j][i].toString();
